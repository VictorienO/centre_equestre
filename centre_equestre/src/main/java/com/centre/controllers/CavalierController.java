...
public class CavalierController {
    ...
    private ArrayList<String> infosCaches = new ArrayList<>();
    ...
    public String addCavalier(@ModelAttribute Cavalier cavalier) {
        String prenom = cavalier.getPrenom();
        String nom = cavalier.getNom();
        String mail = cavalier.getMail();
        List<Cavalier> cavaliers = cavalierService.findAll();

        for (int i = 0; i < cavaliers.size() ; i++){
            Cavalier cavalierTest = cavaliers.get(i);
            String prenom_teste = cavalierTest.getPrenom();
            String nom_teste = cavalierTest.getNom();
            String mail_teste = cavalierTest.getMail();

            if (prenom.equals(prenom_teste) && nom.equals(nom_teste) && mail.equals(mail_teste)){
                infosCaches.add(nom);
                infosCaches.add(prenom);
                infosCaches.add(mail);
                infosCaches.add(cavalier.getDate_naiss());
                infosCaches.add(String.valueOf(cavalier.getNiveau()));
                infosCaches.add(String.valueOf(i));
                return "cavaliers/existingCavalier"; // Redirection vers la vue existingCavalier
            }
        }
        cavalierService.save(cavalier);
        return "redirect:/cavaliers"; // Redirection vers la liste des cavalier
    }

    @PostMapping("/ignorerEtAjouter")
    public String ignorerEtAjouter() {
        Cavalier cavalier = new Cavalier(infosCaches.get(0), infosCaches.get(1), infosCaches.get(2), infosCaches.get(3), Integer.parseInt(infosCaches.get(4)), null);
        cavalierService.save(cavalier);
        infosCaches.clear();
    ...
    public String fusionner() {
        int indice = Integer.parseInt(infosCaches.get(5));
        List<Cavalier> cavaliers_existant = cavalierService.findAll();
        Cavalier old = cavaliers_existant.get(indice);
        int new_niv =Integer.parseInt(infosCaches.get(4));
        old.setNiveau(new_niv);
        cavalierService.edit(old);
        infosCaches.clear();
        return "redirect:/cavaliers";
    }
